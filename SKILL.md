...
- **Running in Cowork** (or any environment with local file access): read and write application files directly in the folder the user has pointed you at. Write the regenerated dashboard back to that same folder, then **offer to pin it to the user's Artifacts library as a standard part of this step, every time it's the first dashboard produced in a chat – don't wait to be asked separately.** Confirmed live: without this offer, a real user has no way to know the extra step exists, and the dashboard stays a local file only. State plainly what this actually is before publishing (see the next bullet), then publish once the user confirms.
- **Cowork's dashboards are pinned, persisted snapshots – not self-refreshing "Live Artifacts" in the connector sense.** Confirmed live, from Cowork's own explanation during a real test: genuine auto-refresh only happens when an Artifact calls a connector tool (Slack, Gmail, a calendar, etc.) live from the browser. This skill's data is local markdown files with no connector behind it, so a job-pipeline dashboard **cannot** auto-refresh that way – it can only be pinned to the Artifacts library as a snapshot that gets pushed forward each time the skill regenerates and republishes it. Real, correctly-described benefits over Chat mode: it persists across sessions, doesn't need a manual Publish click, and updating it never needs a download/re-upload round-trip. Never describe it as "auto-updating," "always-current," or "live" in the sense of updating itself unprompted – say "pinned and persisted, updated whenever the dashboard is regenerated" instead. Push updates to the *same* pinned Artifact on every subsequent regeneration, not a new one each time.
