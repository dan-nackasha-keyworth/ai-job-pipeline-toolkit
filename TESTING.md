...
This repo's skill logic was actually executed, not just written and left unverified. Thirty-two tests, run across sessions between 2026-07-09 and 2026-07-20, documented here as evidence rather than assertion.
...
**Verification took real work, and surfaced exactly why the "don't trust the chat's own claim" discipline matters, even on a genuine success:** the first two screenshots checked were the same native-app local preview seen in Test 30's failure – one showing a stale score (84, from the very first attempt days earlier, not this run's 79), the second showing the correct score but still no visible address bar to confirm it was actually `claude.ai`-hosted rather than a reloaded local file. Only a screenshot of the real Artifacts gallery – reached via the actual browser, not the chat's preview panel – gave unambiguous confirmation: a tile titled "Job Pipeline Dashboard," tagged **Cowork**, "Created 5 minutes ago," sitting correctly alongside the user's two real trackers as a separate, standalone item, not duplicating either.

**What this proves / doesn't prove:** proves the explicit-first-prompt pattern, combined with the `SKILL.md` fixes from Tests 30-31, can produce a genuine, verified, pinned Cowork Artifact – the first time this exact end state has been confirmed with certainty rather than inferred from a chat transcript. Doesn't prove the underlying automatic-offer behaviour (asking with no explicit prompt) is fixed – this run always used the recommended explicit pattern, which is exactly what `README.md` now tells users to do, so this test validates the recommended workflow, not the skill's unprompted behaviour. The stale/ambiguous local-preview screenshots along the way weren't a new bug – ordinary caching, the same class of thing seen earlier with GitHub Pages' `Last-Modified` delay – but they're a real demonstration of why verifying via the actual Artifacts library, not the chat's own account of what it did, is the discipline that caught every previous false or incomplete claim in this whole thread.
