...
**Two real, previously-undocumented gaps found, both fixed as a direct result:**
...
**What this doesn't yet prove:** both fixes above were written *after* this test ran, so the skill actually exercised in this test was the pre-fix version – the `application_materials` field and the explicit read-only reminder haven't themselves been live-tested yet. That requires re-uploading the updated `job-pipeline.zip` to the same test Project and re-running an application-logging conversation, which needs a human to complete the upload step (see the zip-upload limitation noted in Test 13's packaging step above – the browser automation used for this test can drive every step except selecting a file in a native OS picker).
